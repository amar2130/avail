...
pub fn json_subscriber(log_level: Level) -> FmtSubscriber<DefaultFields, Format<Json>> {
...
pub fn default_subscriber(log_level: Level) -> FmtSubscriber<DefaultFields, Format<Full>> {
	FmtSubscriber::builder()
		.with_max_level(log_level)
...
pub fn parse_log_level(log_level: &str, default: Level) -> (Level, Option<ParseLevelError>) {
	log_level
		.to_uppercase()
		.parse::<Level>()
...
pub async fn run(
	...
	cfg: RuntimeConfig,
	need_server: bool,
) -> Result<(Arc<Mutex<types::State>>, Arc<DB>)> {
